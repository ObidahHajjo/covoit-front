...
| 🎟 **الحجز والإدارة**     | حجز المقاعد ومراجعة وإلغاء الحجوزات مع عرض تفاصيل مركبة السائق                  |
...
| 🆘 **الدعم**              | دردشة مباشرة مع المشرف ودعم بالبريد الإلكتروني مع التحقق من المرفقات في الواجهة |
| 🛡 **الإدارة**            | لوحة تحكم الإدارة والمستخدمون والرحلات والمركبات والعلامات والموديلات والدعم    |
...
🎟  الحجوزات .............. حجز مقعد · إلغاء حجز · عرض تفاصيل مركبة السائق
...
🆘  الدعم ................. دردشة مباشرة مع المشرف · نموذج بريد إلكتروني · FAQ مدمجة · التحقق من المرفقات (5 ملفات / 10 ميجابايت)
🛡  الإدارة ............... لوحة التحكم · المستخدمون · الرحلات · العلامات · الموديلات · السيارات · الدعم
...
| --------------------------- | ------------------------------- | ----------------------------------------------------- |
| كوكيز httpOnly من الباك-إند | `access_token`, `refresh_token` | جلسة المصادقة — لا تُقرأ أبداً من JavaScript          |
| `localStorage`              | `covoit.language`               | تفضيل لغة المستخدم                                    |
...
> نقاط نهاية المصادقة تنشئ الجلسة وتجدّدها الآن عبر كوكيز httpOnly فقط. لم تعد الواجهة الأمامية تستهلك رموز المصادقة من استجابات JSON.
...
support.session.{id}          ← دردشة الدعم من جهة المستخدم
support.admins                ← صندوق دعم المشرف
```

تستمع إلى `.chat.message.sent` و`.support.message.sent` و`.support.session.created` بحسب القناة.
...
| 🔐 المصادقة     | تعتمد حصراً على كوكيز httpOnly من الباك-إند وسياسة CORS وتكوين الجلسة          |
...
تعتمد تدفقات الملف الشخصي والحساب على استعادة مستخدم صالح عبر `/auth/me`. إذا فشلت هذه الشاشات:

- تحقّق من أن كوكيز الجلسة تُرسل إلى الباك-إند
- تحقّق من أن `/auth/me` يعيد `user.person.id`
- بعد التسجيل، تحقّق من نجاح استعادة الجلسة قبل إعادة التوجيه إلى `/complete-profile`
...
3. جلسة المتصفح المصادَق عليها صالحة لتفويض القنوات الخاصة `chat.*` و`support.*`
...
### 🚗 تفاصيل المركبة لا تظهر في تفاصيل الرحلة أو الحجز

تعرض الواجهة الأمامية العلامة التجارية والموديل ولوحة السيارة واللون فقط عندما يعيد الـ API `trip.driver.car` مع `model.brand` و`color.hex_code`.

- تحقّق من استجابة الباك-إند للمسار `GET /trips/:id`
- تأكّد من أن `TripResource` يعيد `driver.car`
- إذا ظهر اسم اللون بدون مربع اللون، فتحقّق من وجود `color.hex_code`
