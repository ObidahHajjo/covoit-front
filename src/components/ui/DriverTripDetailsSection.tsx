  ...
  return (
    <div className="rounded-xl border border-[#eee] bg-white p-5">
      <div className="flex items-center gap-3">
        <span className="flex h-9 w-9 items-center justify-center rounded-lg border border-[#eee] bg-[#fafafa] text-sm">{icon}</span>
        <h3 className="text-base font-medium text-[#222]">{title}</h3>
      </div>
      <div className="mt-4 space-y-2">
        {rows.map(({ label, value }) => (
          <div key={label} className="flex items-start justify-between gap-4 rounded-lg border border-[#eee] bg-[#fafafa] px-3 py-2">
            <span className="text-xs font-medium uppercase tracking-wider text-[#888]">{label}</span>
            <span className="text-right text-sm text-[#222]">{value ?? "-"}</span>
          </div>
        ))}
      </div>
    </div>
...
function PassengerRow({ passenger, contactPath }: { passenger: Person; contactPath: string }) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-[#eee] bg-white px-4 py-4 sm:flex-row sm:items-center">
      <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-[#eee] bg-[#fafafa] text-sm font-medium text-[#222]">
        {(passenger.first_name?.[0] ?? passenger.pseudo?.[0] ?? "?").toUpperCase()}
      </div>
      <div className="min-w-0 flex-1">
        <p className="truncate font-medium text-[#222]">{name}</p>
        <p className="text-xs text-[#888]">{passenger.pseudo ? `@${passenger.pseudo}` : `#${passenger.id}`}</p>
      </div>
      <Link
        to={contactPath}
        className="inline-flex shrink-0 items-center justify-center rounded-lg border border-[#eee] bg-[#fafafa] px-4 py-2 text-xs font-medium text-[#222] transition hover:border-[#ccc] hover:bg-white"
      >
        Contact passenger
      </Link>
    </div>
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-0">
      <section className="overflow-hidden rounded-2xl border border-[#eee] bg-white px-5 py-6 sm:px-7 sm:py-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-wider text-[#888]">Driver trip desk</p>
            <h1 className="mt-3 text-3xl font-medium leading-tight text-[#222] sm:text-4xl">{from} - {to}</h1>
            <p className="mt-4 text-sm leading-6 text-[#666] sm:text-base">Manage route details, passenger outreach, and cancellation from one composed view.</p>
          </div>
          <span className="inline-flex rounded-lg border border-[#eee] bg-[#fafafa] px-4 py-2 text-sm font-medium text-[#666]">Trip #{trip.id}</span>
        </div>

        {error ? (
          <div className="mt-6 rounded-lg border border-[#eee] bg-[#fafafa] px-4 py-3 text-sm font-medium text-[#222]">
            {error}
          </div>
        ) : null}
        ...
        <div className="mt-8 rounded-xl border border-[#eee] bg-white p-5 sm:p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              <p className="text-xs font-medium uppercase tracking-wider text-[#888]">Passenger list</p>
              <h2 className="mt-2 text-lg font-medium text-[#222]">Traveling with you</h2>
            </div>
            <span className="rounded-lg border border-[#eee] bg-[#fafafa] px-3 py-1 text-xs font-medium text-[#666]">{passengers.length}</span>
          </div>
          ...
          {passengers.length === 0 ? (
            <div className="mt-5 rounded-xl border border-dashed border-[#eee] bg-[#fafafa] px-6 py-10 text-center">
              ...
              <p className="mt-4 text-lg font-medium text-[#222]">No passengers yet.</p>
              <p className="mt-1 text-sm text-[#888]">New reservations will appear here as they come in.</p>
            </div>
          ) : (
        ...
        <button
          ...
          disabled={cancelling}
          className="mt-8 w-full rounded-lg border border-[#222] bg-[#222] px-4 py-3.5 text-sm font-medium text-white transition hover:bg-[#444] disabled:opacity-40"
        >
